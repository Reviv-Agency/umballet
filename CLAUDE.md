...
- **There is no automated scraper in this repo** — the `content/*.md` stubs in the notched
  repo were a *failed* fetch (`[FETCH ERROR:]`), and `wix-export/` is git-ignored/absent. If
  you have/build a scrape tool, drop it under `tools/` and document it here.
...
5. **An existing widget needs a tweak to fit → change it ADDITIVELY only.** Add a new control
   ...
   **make a new `-v2` widget instead** — never edit a widget in a way that mutates pages already
   using it (the additive rule, §5). Bump `AEW_VERSION` + flush after any widget change.
6. **Confirm before building** if a section's widget match is ambiguous, and report which
   sections reused a widget, which got a new widget, and which needed an additive tweak.
